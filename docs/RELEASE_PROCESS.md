...
5. **Tag** on `main`:
   ...
   60-second-per-target fuzz seed campaign, Tier-1 sanitizers, the
   installed-candidate consumer smoke gate, a hosted five-workload
   downstream matrix, and an explicit perf/RSS runner availability status on
   the tagged commit. The consumer gate compiles and runs the supported easy
   and advanced-session consumers against the exact installed candidate and
   checks loader provenance. The downstream
   ...
   The aggregate gate fails on any missing or unsuccessful required job.
   After it passes, the workflow builds the deterministic source archive,
   attests its provenance, downloads the attestation bundle, signs the archive
   with Sigstore keyless signing, verifies that signature, and publishes the
   archive, checksums, signature, certificate, bundle, and provenance to the
   GitHub Release. It then uploads the artifact set to the Actions run for 35
   days. The SBOM and ABI manifest
...
5. **Review and publish the GitHub Release.** The tag workflow creates it as
   a **draft** with the CHANGELOG section as its body and attaches the signed
   artefacts, so the publish decision stays with the maintainer:
   ```bash
   gh release edit vX.Y.Z --draft=false
   ```
   Pre-creating the release by hand also works; the workflow leaves an existing
   release's body untouched and never overwrites an already-published asset.
   To do that, write the notes to a file and check it is not empty first —
   `extract-changelog-section.sh` emits nothing and exits 0 when the heading
   lacks its ` - YYYY-MM-DD` suffix, which would publish a release with no
   body:
   ```bash
   ./scripts/release/extract-changelog-section.sh X.Y.Z > /tmp/notes.md
   test -s /tmp/notes.md
   ...
   If an upload is interrupted, the next run **refuses** rather than completing
   the set: the `.sig`, `.pem` and `.cosign.bundle` come from one ephemeral key
   and one Rekor entry, so mixing them across runs yields a certificate and a
   signature that verify individually but not against each other. Delete those
   three assets from the release and re-run the job; the deterministic tarball,
   checksums and attestation can be left in place.

   The release body MUST be the verbatim CHANGELOG section for
   `[X.Y.Z]`.  `scripts/ci/check-release-template.sh` is intended to enforce
   equality but does not: no job that runs it supplies a `GH_TOKEN`, so it
   cannot fetch the release body, and it takes its skip branch — which exits 0,
   so meson reports the test as OK rather than SKIP. On a tag push it also runs
   long before the release exists. Tracked in #1288.
6. **Release artefacts** are attached by the tag workflow: the source
   tarball, its checksums, the Sigstore signature (`.sig`), certificate
   (`.pem`) and bundle (`.cosign.bundle`), and the provenance attestation.
   Attaching the SBOM and ABI manifest is still manual; see step 4 for why.
   ...
   Pass `--signature <file> --certificate <file>` for the Sigstore check and
   `--attestation <file>` for the GitHub provenance check; both are published
   from `vX.Y.Z` onward. `--tag` additionally runs `git verify-tag` and needs
   the maintainer GPG key, which is not yet established (#1154).

   Verifying a keyless signature is never offline: the Fulcio certificate is
   valid for about ten minutes, and cosign establishes that the signature was
   made while it was valid from the Rekor entry. The detached `.sig`/`.pem`
...
7. **Retain the downstream evidence.** The tag workflow's
   `Tag / five-workload downstream matrix (DOOP in perf-nightly)` job is a
   release-blocking check. It builds and verifies the exact candidate tarball,
   then runs the pinned five-workload matrix on a GitHub-hosted Ubuntu runner;
   DOOP is explicitly skipped here and remains covered by `perf-nightly`.
   Link the successful workflow run and its 90-day evidence artifact from the
...
- #750 — GPG and cosign keyless signing pipeline, and the key-management
  runbook; the GPG half is executed under #1154.
- #1154 — maintainer signing identity; GPG tag signing and end-to-end
  validation of both signing flows.
- #1287 — anchoring the Sigstore certificate-identity pattern.
