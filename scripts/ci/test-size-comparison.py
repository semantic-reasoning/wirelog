...
def run(args, cwd=None, check=True, **kwargs):
    ...
    return subprocess.run(args,cwd=cwd,text=True,capture_output=True,check=check,**kwargs, encoding="utf-8")
...
with tempfile.TemporaryDirectory(prefix="wirelog-size-merge-fixture-") as temp_name:
    ...
    (repo/"tests/baseline_size.provenance.json").write_text(json.dumps({"schema_version":1,"status":"legacy-unverified","baseline_bytes":100}), encoding="utf-8")
    ...
    doc_report=json.loads(report.read_text(encoding="utf-8"))
