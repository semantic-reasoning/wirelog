...
static int
eval_fingerprint(const char *src, uint32_t num_workers, uint64_t *out_fp,
                 int64_t *out_count)
{
    wirelog_error_t err;
...
static const char *TC_SRC = ".decl edge(x: int32, y: int32)\n"
...
test_k_fusion_correctness_K2(void)
{
    ...
    char msg[128];
    snprintf(msg, sizeof(msg), "K=2 count %" PRId64 " != K=1 count %" PRId64,
             cnt2, g_count_k1);
...
test_k_fusion_correctness_K4(void)
{
    ...
    char msg[128];
    snprintf(msg, sizeof(msg), "K=4 count %" PRId64 " != K=1 count %" PRId64,
             cnt4, g_count_k1);
...
test_k_fusion_correctness_K8(void)
{
    ...
    char msg[128];
    snprintf(msg, sizeof(msg), "K=8 count %" PRId64 " != K=1 count %" PRId64,
             cnt8, g_count_k1);
...
test_k_fusion_correctness_cyclic(void)
{
    ...
    printf("(cnt=%" PRId64 " fp2=%s fp4=%s fp8=%s) ", cnt1,
           fp2 == fp1 ? "match" : "MISMATCH", fp4 == fp1 ? "match" : "MISMATCH",
