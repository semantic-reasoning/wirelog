...
test_shared_view_relation_metadata(void)
{
    ...
    for (long fail_at = 0; fail_at < 64 && !completed; fail_at++) {
        ...
        candidate->compound_arity_len = 1u;
...
test_staged_replacement_contract(void)
{
...

    {
        ...
        CHECK(dst->retained_reserved_bytes == reserved_after
            ...
            == reserved_after + dst->descriptor_reserved_bytes
...

    {
        ...
        CHECK(wl_columnar_source_access_writer_acquire(
            ...
            "locked replacement retry writer admission");
        replacement.writer_acquired = true;
        CHECK(col_rel_prepare_replacement_locked(dst, candidate, &replacement)
            == 0,
        ...
        CHECK(!replacement.writer_acquired && !replacement.reservation_active
            ...
            && wl_columnar_memory_reserved(
                wl_columnar_memory_governor_ref_get(ref))
            == prepared_bytes + dst->descriptor_reserved_bytes
            + dst->metadata_reserved_bytes,
...
test_staged_replacement_prepared_window(void)
{
    const uint64_t descriptor_bytes = sizeof(col_rel_t)
        + sizeof("generation_test")
        + sizeof(wl_columnar_memory_reservation_t);
    const uint64_t one_column_metadata_bytes = 3u + sizeof(char *)
...
#define REPLACEMENT_CHECK(condition, message) \
    ...
    REPLACEMENT_CHECK(dst->relation_identity == dst_identity_before
        ...
        == planned_bytes + dst->descriptor_reserved_bytes
        + dst->metadata_reserved_bytes
