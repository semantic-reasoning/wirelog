...
static int
test_bdx_seed_observed_allocation_failure_is_atomic(void)
{
#ifdef WL_TEST_ALLOC_WRAP
    TEST("BDX seed observed allocation failure is atomic");
...
{
    TEST("TDD merge stages each relation before publication");
...
test_tdd_merge_schema_mismatch_rollback(void)
{
    ...
    /* SIDE relations also carry arity maps; matching maps merge, while a
     * different but individually valid physical grouping is rejected. */
    {
        const int64_t side_target_rows[] = { 9, 9 };
        const int64_t side_worker_rows[] = { 2, 3 };
        col_rel_t *side_target = make_seed_relation("side",
                side_target_rows, 1, 2);
        col_rel_t *side_worker = make_seed_relation("side",
                side_worker_rows, 1, 2);
        col_rel_t *side_workers[] = { side_worker };

        if (!side_target || !side_worker) {
            col_rel_destroy(side_target);
            col_rel_destroy(side_worker);
            col_rel_destroy(target);
            col_rel_destroy(worker);
            FAIL("SIDE metadata setup");
            return 1;
        }
        side_target->compound_kind = WIRELOG_COMPOUND_KIND_SIDE;
        ...
        if (wl_columnar_eval_test_tdd_merge(&side_target, side_workers, 1)
            ...
            col_rel_destroy(target);
            col_rel_destroy(worker);
            FAIL("matching SIDE metadata merge");
            return 1;
        }
        ...
        if (!side_worker->compound_arity_map) {
            ...
            col_rel_destroy(target);
            col_rel_destroy(worker);
            FAIL("mismatched SIDE arity map allocation");
            return 1;
        }
        ...
        if (wl_columnar_eval_test_tdd_merge(&side_target, side_workers, 1)
            ...
            col_rel_destroy(target);
            col_rel_destroy(worker);
            FAIL("mismatched SIDE metadata changed target");
            return 1;
        }
        col_rel_destroy(side_target);
        col_rel_destroy(side_worker);
...
main(void)
{
    ...
    test_bdx_seed_observed_allocation_failure_is_atomic();
