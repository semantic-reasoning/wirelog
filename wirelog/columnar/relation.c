...
static int col_rel_cow_unshare_impl(col_rel_t *r, uint32_t new_cap,
    bool defer_alias_release, bool preserve_table_charge);
...
col_rel_set(col_rel_t *r, uint32_t row, uint32_t col, int64_t val)
{
    ...
    if (r->col_shared) {
        rc = col_rel_cow_unshare_impl(r, 0, true, false);
        if (rc != 0)
            goto release_writer;
...
static bool
col_rel_shared_table_bytes(uint32_t ncols, bool flags, uint64_t *out)
{
    uint64_t lane = sizeof(int64_t *) + (flags ? sizeof(bool) : 0u);
    return wl_columnar_memory_size_mul(ncols, lane, out)
           && *out <= SIZE_MAX;
}

/* A replacement table overlaps the old table during preparation.  Keep the
 * old token committed and reserve the complete new shape until the old
 * storage has physically been retired. */
static int
col_rel_begin_shared_table_replacement(col_rel_t *r)
{
    uint64_t bytes;
    wl_columnar_memory_admission_status_t status;
    if (!r->shared_table_reservation)
        return 0;
    if (!col_rel_shared_table_bytes(r->ncols, false, &bytes)
        || !wl_columnar_memory_size_add(bytes,
        sizeof(*r->shared_table_reservation), &bytes))
        return EOVERFLOW;
    status = wl_columnar_memory_begin_replacement(
        r->shared_table_reservation, bytes);
...
static void
col_rel_finish_shared_table_replacement(col_rel_t *r, bool publish)
{
    if (!r->shared_table_reservation)
        return;
    if (publish) {
        if (!wl_columnar_memory_commit_replacement(
                r->shared_table_reservation))
            abort();
    } else if (!wl_columnar_memory_rollback_replacement(
            r->shared_table_reservation)) {
        abort();
    }
}
...
static void
col_rel_release_shared_table(col_rel_t *r)
{
    if (!r->shared_table_reservation)
        return;
    if (!wl_columnar_memory_release(r->shared_table_reservation))
        abort();
    free(r->shared_table_reservation);
    r->shared_table_reservation = NULL;
}

static int
col_rel_prepare_resize(col_rel_t *r, uint32_t new_cap,
    int64_t ***out_columns, col_delta_timestamp_t **out_timestamps)
{
    ...
    if (!r || !out_columns || !out_timestamps)
        return EINVAL;
    int table_rc = col_rel_begin_shared_table_replacement(r);
    if (table_rc != 0)
        return table_rc;
    if (r->ncols > 0) {
        columns = col_columns_alloc(r->ncols, new_cap);
        if (!columns) {
            col_rel_finish_shared_table_replacement(r, false);
            return ENOMEM;
        }
...
col_rel_release_column_storage(col_rel_t *r)
{
    ...
    r->columns = NULL;
    r->col_shared = NULL;
    /* Callers that replace the table settle its replacement token after
    ...
    col_delta_timestamp_t *timestamps, uint32_t new_cap)
{
    ...
    r->capacity = new_cap;
    r->arena_owned = false;
    col_rel_finish_shared_table_replacement(r, true);
}
...
/* Flat schema copies own fresh names, type lanes, and Arrow children.  The
 * source's nested Arrow tree is not copied by these publication paths. */
static bool
col_rel_flat_copy_metadata_bytes(const col_rel_t *owner,
    const col_rel_t *source, bool force_names, bool force_schema,
    uint64_t *out)
{
    uint64_t bytes = 0;
    uint64_t part;
    ...
    if (!owner || !source || !out)
        return false;
    names = force_names || source->col_names != NULL;
    schema = force_schema || source->schema_ok;
    if (owner->pool_owned && owner->name
        && !wl_columnar_memory_size_add(bytes,
        strlen(owner->name) + 1u, &bytes))
        return false;
    if (names && source->ncols > 0
        && (!wl_columnar_memory_size_mul(source->ncols,
        sizeof(*source->col_names), &part)
        || !wl_columnar_memory_size_add(bytes, part, &bytes)))
        return false;
    if (source->column_types && source->ncols > 0
        && (!wl_columnar_memory_size_mul(source->ncols,
        sizeof(*source->column_types), &part)
        || !wl_columnar_memory_size_add(bytes, part, &bytes)))
        return false;
    if (schema) {
        if (!wl_columnar_memory_size_add(bytes, 3u, &bytes)
            || !wl_columnar_memory_size_mul(source->ncols,
            sizeof(struct ArrowSchema *) + sizeof(struct ArrowSchema)
            + 2u, &part)
            || !wl_columnar_memory_size_add(bytes, part, &bytes))
    ...
    for (uint32_t c = 0; c < source->ncols; c++) {
        const char *name = source->col_names ? source->col_names[c] : NULL;
        if (names && (name || force_names)
            && !wl_columnar_memory_size_add(bytes,
            name ? strlen(name) + 1u : 1u, &bytes))
            return false;
    ...
    if (source->compound_kind != WIRELOG_COMPOUND_KIND_NONE) {
        if (!col_rel_compound_map_bytes(source, &part)
            || !wl_columnar_memory_size_add(bytes, part, &bytes))
            return false;
    }
    *out = bytes;
...
{
    return col_rel_cow_unshare_impl(r, new_cap, false, false);
}
    ...
    bool defer_alias_release, bool preserve_table_charge)
{
    ...
    private_cols = NULL;
    {
        ...
        if (!any_shared) {
            free(r->col_shared);
            r->col_shared = NULL;
            if (r->shared_table_reservation && !preserve_table_charge) {
                uint64_t bytes;
                if (!col_rel_shared_table_bytes(r->ncols, false, &bytes)
                    || !wl_columnar_memory_size_add(bytes,
                    sizeof(*r->shared_table_reservation), &bytes)
    ...
    const wl_columnar_source_access_writer_t *writer)
{
    ...
    if (writer->owner != &owner->source_access
        ...
        return EINVAL;
    return col_rel_cow_unshare_impl(r, 0, true, false);
}
    ...
    wl_columnar_memory_governor_ref_t *memory_governor)
{
    ...
    wl_columnar_memory_reservation_t table_pending;
    wl_columnar_memory_reservation_t *owned = NULL;
    wl_columnar_memory_reservation_t *metadata_owned = NULL;
    wl_columnar_memory_reservation_t *table_owned = NULL;
    wl_columnar_memory_admission_status_t status;
    uint64_t bytes;
    ...
    if (r->col_shared) {
        ...
        if (!col_rel_shared_table_bytes(r->ncols, true,
            &table_bytes)) {
            col_rel_discard_metadata_reservation(&metadata_pending,
                metadata_owned);
            return EOVERFLOW;
        }
        int table_rc = col_rel_reserve_metadata(r, memory_governor,
                table_bytes, &table_pending, &table_owned);
        if (table_rc != 0) {
            col_rel_discard_metadata_reservation(&metadata_pending,
                metadata_owned);
            return table_rc;
    ...
    * Adopt its already-owned descriptor and name as one transaction. */
    if (!r->pool_owned) {
        ...
        if (!wl_columnar_memory_size_add(sizeof(*r),
            ...
            col_rel_discard_metadata_reservation(&metadata_pending,
                metadata_owned);
            if (table_owned)
                col_rel_discard_metadata_reservation(&table_pending,
                    table_owned);
            return EOVERFLOW;
        }
        wl_columnar_memory_reservation_init(&pending);
    ...
    if (metadata_owned
        ...
        abort();
    if (table_owned
        && (!wl_columnar_memory_reservation_move(table_owned,
        &table_pending)
        || !wl_columnar_memory_commit(table_owned, r)))
        abort();
    ...
    r->shared_table_reservation = table_owned;
    wl_columnar_memory_reservation_init(&r->retained_reservation);
    ...
    bool preserve_descriptor_admission)
{
    ...
    r->columns = NULL;
    r->col_shared = NULL;
    col_rel_release_shared_table(r);
    ...
    wl_columnar_source_access_writer_t *writer, bool writer_held)
{
    ...
    if (r->col_shared) {
        if (col_rel_cow_unshare_impl(r, 0, true, false) != 0)
            goto enomem;
        alias_release_pending = true;
    }
#ifdef WL_TEST_APPEND_HOOK
    ...
    bool *out_alias_release_pending)
{
    ...
    *out_alias_release_pending = false;
    ...
    if (r->col_shared && required <= r->capacity) {
        int rc = col_rel_cow_unshare_impl(r, 0, true, false);
        if (rc == 0)
...
wl_columnar_relation_delta_detach(col_rel_t *rel, uint64_t expected_identity)
{
    ...
    free(rel->col_shared);
    col_rel_release_shared_table(rel);
    rel->columns = NULL;
    rel->col_shared = NULL;
    ...
    uint32_t nrows, uint32_t ncols, bool overwrite)
{
    ...
    rc = col_rel_begin_shared_table_replacement(rel);
    if (rc != 0)
        goto done;
    columns = col_columns_alloc(ncols, nrows);
    if (!columns) {
        rc = ENOMEM;
        col_rel_finish_shared_table_replacement(rel, false);
        goto done;
    }
    ...
    col_rel_finish_shared_table_replacement(rel, true);
#ifdef WL_TEST_RELATION_RESIZE_HOOK
...
col_rel_append_all(col_rel_t *dst, const col_rel_t *src, wl_arena_t *arena)
{
    ...
    if (!dst->column_types && src->column_types) {
        if (!col_rel_prospective_metadata_bytes(dst, dst->ncols,
            (const char *const *)dst->col_names, dst->col_names != NULL,
            false, true, &metadata_bytes)) {
            rc = EOVERFLOW;
            goto cleanup;
        }
    ...
    if (dst->col_shared) {
        rc = col_rel_cow_unshare_impl(dst, 0, true, false);
        if (rc != 0)
            goto cleanup;
        alias_release_pending = true;
    }
    ...
    if (prepared_schema_ok) {
        ...
        dst->schema_ok = true;
        previous_metadata = col_rel_publish_metadata_reservation(dst,
                &metadata_pending, metadata_owned);
        metadata_owned = NULL;
        col_rel_release_metadata_reservation(previous_metadata);
        prepared_types = NULL;
...
col_rel_install_shared_view_unprotected(col_rel_t *dst, const col_rel_t *src)
{
    ...
    wl_columnar_memory_reservation_t metadata_pending;
    wl_columnar_memory_reservation_t *metadata_owned = NULL;
    wl_columnar_memory_reservation_t shared_table_pending;
    wl_columnar_memory_reservation_t *shared_table_owned = NULL;
    uint64_t metadata_bytes;
    uint64_t shared_table_bytes = 0;
    ...
    if (!col_rel_flat_copy_metadata_bytes(dst, src, true, true,
        &metadata_bytes))
        return EOVERFLOW;
    prepare_rc = col_rel_reserve_metadata(dst, dst->memory_governor,
            metadata_bytes, &metadata_pending, &metadata_owned);
    if (prepare_rc != 0)
        return prepare_rc;
    if (!col_rel_shared_table_bytes(dst->ncols, true,
        &shared_table_bytes)) {
        prepare_rc = EOVERFLOW;
        goto prepare_fail;
    }
    prepare_rc = col_rel_reserve_metadata(dst, dst->memory_governor,
            shared_table_bytes, &shared_table_pending,
            &shared_table_owned);
    if (prepare_rc != 0)
        goto prepare_fail;
    prepare_rc = ENOMEM;
    if (shared_arity_len > 0) {
    ...
    for (uint32_t c = 0; c < dst->ncols; c++) {
        ...
        if (ArrowSchemaSetName(prepared_schema.children[c],
            shared_names ? shared_names[c] : "") != NANOARROW_OK) {
            prepare_rc = ENOMEM;
            goto prepare_fail;
        }
    }
    {
        ...
        free((void *)old_columns);
        free(old_shared);
        col_rel_release_shared_table(dst);
        free(dst->column_types);
        free(dst->timestamps);
        ...
        if (shared_table_owned) {
            if (!wl_columnar_memory_reservation_move(shared_table_owned,
                &shared_table_pending)
                || !wl_columnar_memory_commit(shared_table_owned, dst))
                abort();
        }
        dst->shared_table_reservation = shared_table_owned;
...
prepare_fail:
    ...
    if (metadata_owned)
        col_rel_discard_metadata_reservation(&metadata_pending,
            metadata_owned);
    if (shared_table_owned)
        col_rel_discard_metadata_reservation(&shared_table_pending,
            shared_table_owned);
    return prepare_rc;
}
    ...
    const wl_columnar_radix_workspace_t *workspace)
{
    ...
    if (borrowed) {
        ...
        if (col_rel_cow_unshare_impl(r, 0, true, true) != 0) {
            free((void *)old_columns);
            free(old_shared);
            return ENOMEM;
        }
...
wl_columnar_relation_rename_checked(col_rel_t *rel, const char *name)
{
    ...
    wl_columnar_source_access_writer_t source_writer = { 0 };
    wl_columnar_memory_reservation_t pending;
    wl_columnar_memory_reservation_t *owned = NULL;
    wl_columnar_memory_reservation_t *previous;
    uint64_t bytes;
    uint64_t name_bytes;
    uint64_t old_name_bytes;
    char *replacement;
    char *old_name;
    col_rel_t *owner = NULL;
    int rc;
    ...
    if (rel->pool_owned) {
        ...
        if (rc == 0)
            rc = wl_columnar_source_access_writer_acquire(
                &owner->source_access, &source_writer);
        if (rc != 0)
            goto done;
    }
    name_bytes = strlen(name) + 1u;
    old_name_bytes = rel->name ? strlen(rel->name) + 1u : 0u;
    if (rel->pool_owned) {
        if (!col_rel_current_metadata_bytes(rel, &bytes)
            || bytes < old_name_bytes
            || !wl_columnar_memory_size_add(bytes - old_name_bytes,
            name_bytes, &bytes)) {
            rc = EOVERFLOW;
            goto done;
        }
    } else if (!wl_columnar_memory_size_add(sizeof(*rel), name_bytes,
        &bytes)) {
        rc = EOVERFLOW;
        goto done;
    }
    rc = col_rel_reserve_metadata(rel, rel->memory_governor, bytes,
            &pending, &owned);
    if (rc != 0)
        goto done;
    ...
    if (rel->pool_owned) {
        previous = col_rel_publish_metadata_reservation(rel, &pending,
                owned);
    } else {
        previous = rel->descriptor_reservation;
        if (owned && (!wl_columnar_memory_reservation_move(owned, &pending)
            || !wl_columnar_memory_commit(owned, rel)))
            abort();
        rel->descriptor_reservation = owned;
    }
    free(old_name);
    col_rel_release_metadata_reservation(previous);
    rc = 0;
done:
    if (source_writer.owner
        && wl_columnar_source_access_writer_release(&source_writer) != 0
        && rc == 0)
        rc = EINVAL;
    if (wl_columnar_source_access_writer_release(&descriptor) != 0
        && rc == 0)
        rc = EINVAL;
    return rc;
}
