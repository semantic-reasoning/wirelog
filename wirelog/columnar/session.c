...
static void
session_rels_release(wl_col_session_t *sess)
{
    if (sess->memory_governor
...
session_pool_rel_transfer_payload(col_rel_t *dst, col_rel_t *src)
{
    ...
    memcpy(dst, src, retained_offset);
...
session_pool_rel_promote(col_rel_t *src, col_rel_t **heap_out)
{
    ...
    *heap_out = NULL;
    ...
    heap = (col_rel_t *)calloc(1, sizeof(*heap));
    if (!heap) {
        rc = ENOMEM;
        goto fail;
    }
...
session_add_rel(wl_col_session_t *sess, col_rel_t *r)
{
    ...
    if (r->pool_owned) {
        col_rel_t *heap = NULL;
        int promote_rc = session_pool_rel_promote(r, &heap);
        if (promote_rc != 0)
            return promote_rc;
        pool_src = r;
        r = heap;
    ...
    if (sess->nrels >= sess->rel_cap) {
        ...
        if (!nr){
            if (sess->memory_governor
                && !wl_columnar_memory_rollback(&pending))
                abort();
            goto oom;
        }
        if (sess->nrels > 0)
        ...
        free((void *)sess->rels);
        session_rels_release(sess);
        if (sess->memory_governor
            && sess->rels_reservation.identity
