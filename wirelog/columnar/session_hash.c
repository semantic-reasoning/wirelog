 ...
 * Returns 0 on success, ENOMEM on allocation failure, or ENOSPC on
 * governor denial. The hash is optional; callers can use linear lookup.
 * Replaces both arrays together so allocation or admission failure leaves
 * the old lookup image intact until the caller selects linear fallback.
 */
int
        ...
        wl_columnar_memory_admission_status_t status
            = wl_columnar_memory_reserve_checked(
                wl_columnar_memory_governor_ref_get(
                    session->memory_governor),
                (uint64_t)head_bytes + (uint64_t)next_bytes,
