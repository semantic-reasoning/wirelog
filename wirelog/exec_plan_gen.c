...
/* ======================================================================== */
/* Multi-Way Delta Expansion (Semi-Naive K-Atom Rewriting)                  */
/* ======================================================================== */
                      ...
                      uint32_t *delta_pos)
{
    ...
    for (uint32_t i = 0; i < op_count; i++) {
        if (ops[i].op == WL_PLAN_OP_VARIABLE) {
            /* Check if relation is IDB (recursive in this stratum) */
            bool is_idb = false;
            ...
            /* Check if right_relation is IDB */
            bool is_idb = false;
            for (uint32_t r = 0; r < idb_count; r++) {
...
clone_plan_op(const wl_plan_op_t *src, wl_plan_op_t *dst)
{
    ...
    /* Clone filter expression buffer */
...
/**
 * Rewrite a single relation plan for multi-way delta expansion.
 ...
 * The K copies are joined by CONCAT ops and a final CONSOLIDATE.
 *
...
expand_multiway_delta(const wl_plan_op_t *ops, uint32_t op_count,
                      ...
                      uint32_t *out_count)
{
    /* Total ops: K copies of original + (K-1) CONCATs + 1 CONSOLIDATE */
    uint32_t total = k * op_count + (k - 1) + 1;
    ...
    for (uint32_t d = 0; d < k; d++) {
        ...
        for (uint32_t i = 0; i < op_count; i++) {
            ...
            /* Set delta_mode for this copy:
             * The delta position d gets FORCE_DELTA, all other delta
             * positions get FORCE_FULL, non-delta ops stay AUTO. */
        ...
        /* After the second+ copy, emit CONCAT to merge with previous */
        if (d > 0) {
    ...
    /* Final CONSOLIDATE to deduplicate the union */
    memset(&new_ops[wi], 0, sizeof(wl_plan_op_t));
 ...
 * replaces the relation's ops with K expanded copies.
 */
static void
rewrite_multiway_delta(wl_plan_t *plan)
{
    for (uint32_t s = 0; s < plan->stratum_count; s++) {
        ...
        /* Collect IDB relation names in this stratum */
...
wl_plan_from_program(const struct wirelog_program *prog, wl_plan_t **out)
{
    ...
    *out = NULL;
     ...
     * For rules with K >= 3 IDB body atoms, emit K copies of the rule
     * plan with each copy forcing exactly one body atom to use its delta
     * relation and the rest to use full relations. */
